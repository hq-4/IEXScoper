...
- `utils/build_dead_ticker_review_queue.py` combines SEC and IEX current evidence with ticker-era continuity to produce the review queue for dead, intermittent, and partial-window symbols that still need historical identity evidence.
...
- The dead ticker review queue lives at `reports/dead-ticker-review/dead_ticker_review_queue.parquet`. It contains `26,184` non-stable ticker eras; `18,434` have no current SEC or IEX evidence and are marked `historical_identity_unresolved`, while the rest have current-only evidence that still needs historical caution.

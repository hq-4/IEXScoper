...
- The unresolved priority queue lives at `reports/dead-ticker-review/unresolved_priority_queue.parquet`. It currently ranks `18,430` unresolved eras; `12,516` are probable operating-company rows under the richer instrument type and `1,860` are delisted/acquired candidates. The default top-100 batch is entirely probable-operating delisted/acquired rows.

...
- OpenFIGI enrichment reads the API key from `OPENFIGI_API_KEY` by default and never logs the key value. The key currently lives in `.env` as `OPENFIGI_KEY`; the identity builder accepts either variable (`--api-key-env` overrides).

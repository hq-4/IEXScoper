...
def get_hist_data(date: str):
    ...
    for file in date_data:
        if (
            file.get("feed") == "DEEP"
            and file.get("date") == date
            and file.get("version") == "1.0"
            and file.get("protocol") == "IEXTP1"
        ):
            return file
...
def download_hist_file(date: str, download_dir: str) -> bool:
    ...
    if os.path.exists(file_path):
        if int(os.path.getsize(file_path)) == int(expected_file_size):
            ...
            print(
                f"Expected file size {expected_file_size}. Current file size {os.path.getsize(file_path)}. Redownloading..."
            )

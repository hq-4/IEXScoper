...
def run_batch(args: argparse.Namespace, batch_input: Path, batch_root: Path, user_agent: str) -> None:
    ...
    run_py(
        "utils/legacy/edgar_full_text_triage.py",
...
def run_verification_steps(args: argparse.Namespace, paths: dict[str, Path], user_agent: str) -> None:
    run_py(
        "utils/legacy/build_lifecycle_override_candidates.py",
        "--template-path", paths["lifecycle_window"],
        "--triage-path", paths["triage_parquet"],
        "--output-path", paths["candidates"],
    )
    run_py(
        "utils/legacy/verify_sec_override_candidates.py",
    ...
    run_py(
        "utils/legacy/build_sec_lifecycle_text_evidence.py",
    ...
    run_py(
        "utils/legacy/build_sec_lifecycle_followup_evidence.py",
...
def regenerate_queues() -> None:
    ...
    run_py("utils/legacy/build_dead_ticker_resolution_lanes.py", "--output-root", "reports/dead-ticker-review/resolution-lanes")
    run_py("utils/legacy/build_dead_ticker_resolution_workplan.py")
...
def run_py(*args: Any) -> None:
    subprocess.run([sys.executable, *(str(arg) for arg in args)], cwd=Path(__file__).resolve().parents[2], check=True)
